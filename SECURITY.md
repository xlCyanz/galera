...
- **Comandos de Tauri.** Toda entrada que cruce desde el webview hacia Rust se valida en el backend. La interfaz no tiene permisos de sistema de archivos ni de diálogos: solo los básicos de Tauri (`core:default`), y una prueba lo comprueba. El backend solo abre carpetas que quien usa la app ha elegido en el diálogo nativo; una ruta mandada desde el webview que no se haya elegido ahí se rechaza, aunque sea un proyecto válido. Cualquier forma de abrir o leer una carpeta que no se haya elegido es una vulnerabilidad.
